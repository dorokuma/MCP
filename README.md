...
`search_web` returns the snippet the search engine picked — around 20 words, often a keyword-bearing fragment that never answers the question. `search_web_deep` also reads each page via [Reader](https://jina.ai/reader), splits it into ~100-word passages at sentence boundaries, and scores every passage from every page in one listwise [Reranker](https://jina.ai/reranker) call, so any page's passage can outrank any other's.

`snippet_source` decides what may come back: `auto` (the default) also enters each page's engine snippet as a candidate, and the response field of the same name says which won; `content` never enters it and omits pages it could not read, so it may return fewer than `num`.

Top 5 per mode from the live server, snippets abridged:

**English — `what is the latest model from jina ai`**

| # | `search_web` | `deep`·`auto` | `deep`·`content` |
|---|---|---|---|
| 1 | **jina.ai/models**<br>We've been moving the needle in search models since day on… | **elastic.co/search-labs/blog/on-p…**<br>`serp` All 28 Jina AI models available, including the latest jina… | **jina.ai**<br>Tech blog Bootstrapping Audio Embeddings from Multimodal L… |
| 2 | **jina.ai**<br>Jina models natively inside Elasticsearch. v3.5: An Effici… | **jina.ai/embeddings**<br>`serp` jina-embeddings-v4 is our latest universal multimodal mode… | **jina.ai/embeddings**<br>arXiv July 20, 2026 jina-reranker-v3.5: An Efficient Listw… |
| 3 | **huggingface.co/jinaai**<br>Jina AI: Embeddings, Rerankers and Small Language Models. … | **huggingface.co/jinaai**<br>`serp` Jina AI: Embeddings, Rerankers and Small Language Models f… | **huggingface.co/jinaai**<br>Recent Activity florian-hoenickeupdated a model about 10 h… |
| 4 | **jina.ai/embeddings**<br>jina-embeddings-v4 is our latest universal multimodal mode… | **jina.ai**<br>`content` Tech blog Bootstrapping Audio Embeddings from Multimodal L… | **cloud.google.com/blog/products/a…**<br>Jina Reader isn't just another scraper; it takes a differe… |
| 5 | **elastic.co/search-labs/blog/on-p…**<br>All 28 Jina AI models available, including the latest jina… | **newrelic.com/instant-observabili…**<br>`content` Early issue detection: Detect and address issues early to … | **jina.ai/models**<br>warning calendar\month 2023-06-17 The first version of the… |

**Chinese — `jina ai 最新的模型是什么`**

| # | `search_web` | `deep`·`auto` | `deep`·`content` |
|---|---|---|---|
| 1 | **jina.ai/zh-TW/about-us**<br>Jina AI 由肖涵博士於2020年創建,是一家領先的搜索AI 公司。我們專注開發向量模型、重排器、Reader和… | **ithome.com.tw/news/159507**<br>`content` Jina AI最新第二代文字嵌入模型jina-embeddings-v2，已可處理多達8,192個token，在多項… | **ithome.com.tw/news/159507**<br>Jina AI最新第二代文字嵌入模型jina-embeddings-v2，已可處理多達8,192個token，在多項… |
| 2 | **jina.ai/zh-TW/news/jina-reader-f…**<br>Grounding 技術對GenAI 應用程式來說至關重要。我們全新的https://s.jina.ai/ 讓LLM… | **jina.ai/zh-CN/embeddings**<br>`content` 两者都与 v5-text 完全兼容——无需重新索引。 v5-text：最新最先进的小型多语言向量模型 jina-em… | **jina.ai/zh-CN/embeddings**<br>两者都与 v5-text 完全兼容——无需重新索引。 v5-text：最新最先进的小型多语言向量模型 jina-em… |
| 3 | **elastic.co/cn/jina-search-models**<br>什么是Jina 搜索模型？ Jina 模型是开源的、前沿的检索AI 模型。它们包括用于向量的嵌入模型、用于提高精确度… | **elastic.co/cn/jina-search-models**<br>`content` 您可以从 semantic_text 开始，或访问各模型子页面，查看代码示例、API 参考和教程。 目前可用的 Ji… | **elastic.co/cn/jina-search-models**<br>您可以从 semantic_text 开始，或访问各模型子页面，查看代码示例、API 参考和教程。 目前可用的 Ji… |
| 4 | **milvus.io/docs/zh-hant/embed-wit…**<br>Jina AI. Jina AI 的嵌入模型是高性能的文字嵌入模型，可以將文字輸入轉換為數字表示，捕捉文字的語義。這… | **milvus.io/docs/zh-hant/embed-wit…**<br>`content` Jina AI’s embedding models are high-performance text embed… | **jina.ai/zh-TW/about-us**<br>我們專注開發向量模型、重排器、Reader和小型語言模型，幫助企業和開發者構建強大的搜索應用程序，提供高相關、紮實、… |
| 5 | **jina.ai/zh-CN/embeddings**<br>v5-omni：一个向量，涵盖所有模态 文本、图像、音频、视频——共享同一个向量空间，两种尺寸。v5-omni-sm… | **jina.ai/zh-TW/about-us**<br>`serp` Jina AI 由肖涵博士於2020年創建,是一家領先的搜索AI 公司。我們專注開發向量模型、重排器、Reader和… | **jina.ai/zh-TW/news/jina-reader-f…**<br>因為阻止企業向數百萬用戶部署 LLMs 的主要障礙是信任度：答案是真實的，還是僅僅是模型的幻覺（hallucinat… |

- **The engine snippet is sometimes the better answer.** English `auto`'s top three are all `serp`, and the first answers the query more directly than any extracted passage — while `content` promotes jina.ai homepage navigation instead. Prefer `auto` unless something downstream needs full passages.
...
- **Passages are not uniformly ~100 words.** The median goes from 22 to 106 words in English and 58 to 92 CJK characters in Chinese, but navigation-heavy pages lack sentence punctuation to split on: 272 words for the first English `content` result.
